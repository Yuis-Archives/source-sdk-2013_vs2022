...
int main(int argc, char* argv[])
{
	...
	for(int mode=0;mode<2;mode++)
	{
		...
		if( !fn )
		{
			printf( "vrad_launcher error: can't get factory from yrad_dll.dll\n" );
		...
		if( !pDLL )
		{
			printf( "vrad_launcher error: can't get IVRadDLL interface from yrad_dll.dll\n" );
			Sys_UnloadModule( pModule );
			return 3;
		}
